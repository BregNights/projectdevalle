...
class RemoveOfferServiceTest {
    ...
    private RemoveOfferService service;

    @BeforeEach
    void setUp() {
        service = new RemoveOfferService(offerRepository, ownershipResolver);
    }

    @Test
    void removesTheResolvedOfferAndPersistsIt() {
        UUID userId = UUID.randomUUID();
        UUID offerId = UUID.randomUUID();
        Offer offer = anActiveOffer();
        when(ownershipResolver.resolveOwnedOffer(offerId, userId)).thenReturn(offer);

        service.remove(new OfferOwnershipCommand(offerId, userId));

        assertThat(offer.status()).isEqualTo(OfferStatus.REMOVED);
